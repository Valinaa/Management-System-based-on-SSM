...
public class IndexController {
    ...
    @RequestMapping("/updatePass/{admin}/{password}")
    public String toUpdatePass(@PathVariable("admin") String admin, @PathVariable("password") String password){
    ...
    * 通过实体获取JSON数组
    * */
    public void toCheckAnswer(@RequestBody List<QuesCheck> check, String ExamID, String NUM,HttpServletRequest request,HttpServletResponse response) throws IOException {
        ...
        * 计算总分与实际得分
        * */
        for(Iterator<QuesCheck> itQ= check.iterator();itQ.hasNext();){
            QuesCheck qc=itQ.next();
    ...
    public String getMainView(Model model) {
        ...
        /*
        List<Escore> score=escoreService.selectAll();
        List<String> num1=new ArrayList<>();
        List<String> num2=new ArrayList<>();
        List<String> num3=new ArrayList<>();
        List<String> num4=new ArrayList<>();
        for(Iterator<Escore> it=score.iterator(); it.hasNext();){
            Escore e=it.next();
            if(e.getExamId()==1){
                num1.add(e.getsNumber());
            }
            if(e.getExamId()==2){
                num2.add(e.getsNumber());
            }
            if(e.getExamId()==3){
                num3.add(e.getsNumber());
            }
            if(e.getExamId()==4){
                num4.add(e.getsNumber());
            }
        }
